...
server <- function(input, output, session) {
  output$plot <- renderPlotly({
    ...
    p <- p + theme(panel.grid.minor.x = element_blank(), panel.background = element_rect(fill="gray60"),
  ...
  output$rp_qb <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Slot == 'QB' & as.integer(pdf$Week) <= as.integer(input$rp_wk))
  ...
  output$rp_rb <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Position == 'RB' & 
                    as.integer(pdf$Week) <= as.integer(input$rp_wk) & pdf$Slot != 'BE' & pdf$Slot != 'IR')
    
    #wdf$QB.Points <- as.numeric(unlist(tapply(wdf$Points, wdf$Team, cumsum)))
    wdf <- wdf %>% group_by(Team, Week) %>% summarize(RB.Points = sum(Points), n = n())
    
    wdf <- wdf %>% group_by(Team) %>% arrange(Week) %>% mutate(RB.Points = cumsum(RB.Points), RBs = cumsum(n))
  ...
  output$rp_wr <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Position == 'WR' & 
                    as.integer(pdf$Week) <= as.integer(input$rp_wk) & pdf$Slot != 'BE' & pdf$Slot != 'IR')
    
    #wdf$QB.Points <- as.numeric(unlist(tapply(wdf$Points, wdf$Team, cumsum)))
    wdf <- wdf %>% group_by(Team, Week) %>% summarize(WR.Points = sum(Points), n = n())
    
    wdf <- wdf %>% group_by(Team) %>% arrange(Week) %>% mutate(WR.Points = cumsum(WR.Points), WRs = cumsum(n))
  ...
  output$rp_te <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Position == 'TE' & 
                    as.integer(pdf$Week) <= as.integer(input$rp_wk) & pdf$Slot != 'BE' & pdf$Slot != 'IR')
  ...
  output$rp_k <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Slot == 'K' & as.integer(pdf$Week) <= as.integer(input$rp_wk))
    #wdf$QB.Points <- as.numeric(unlist(tapply(wdf$Points, wdf$Team, cumsum)))
  ...
  output$rp_dst <- renderPlotly({
    wdf <- subset(pdf, pdf$Year == input$rp_yr & pdf$Slot == 'D/ST' & as.integer(pdf$Week) <= as.integer(input$rp_wk))
    #wdf$QB.Points <- as.numeric(unlist(tapply(wdf$Points, wdf$Team, cumsum)))
  ...
  output$rp_power <- renderTable({
    ...
    if (input$rp_wk == 1) {
      ...
      sdf <- subset(sdf, sdf$Year == input$rp_yr & as.integer(sdf$Week) == as.integer(input$rp_wk)-1)
    }
